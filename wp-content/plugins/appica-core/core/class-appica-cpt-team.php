...
class Appica_CPT_Team {
	...
	public function save_meta_boxes( $post_id, $post ) {
		...
		if ( ! array_key_exists( 'appica_team_nonce_field', $_POST )
		     || ! wp_verify_nonce( $_POST['appica_team_nonce_field'], 'appica_team_nonce' ) ) {
			return;
		}
		...
		// Save subtitle
		$meta_box_name = '_appica_team_subtitle';
		...
		// Save socials
		$meta_box_name = '_appica_team_social';
		$meta_box_value = Appica_Helpers::process_social_networks( $_POST[ $meta_box_name ] );
		update_post_meta( $post_id, $meta_box_name, $meta_box_value );
		unset( $meta_box_name, $meta_box_value );
	}
	...
	public function render_subtitle_mb( $post ) {
		wp_nonce_field( 'appica_team_nonce', 'appica_team_nonce_field' );

		$meta_box_name = '_appica_team_subtitle';
		$meta_box_value = get_post_meta( $post->ID, $meta_box_name, true );

		printf( '<label for="%1$s" class="screen-reader-text">%2$s</label><br>', $meta_box_name, __( 'Drop any line here', 'appica' ) );
		printf(
			'<input type="text" class="%2$s widefat" id="%2$s" name="%2$s" value="%1$s">',
			esc_attr( $meta_box_value ), $meta_box_name
		);
	...
	public function render_social_mb( $post ) {
		...
		$meta_box_name = '_appica_team_social';
		$socials = get_post_meta( $post->ID, $meta_box_name, true );

		echo '<p>', __( 'Choose social networks for displaying', 'appica' ), '</p>';
		echo '<div class="appica-social-networks-wrap">';
		if ( ! empty( $socials ) ) {
			$this->render_networks_list( $meta_box_name, $networks, $socials );
		} else {
			$this->render_empty_list( $meta_box_name, $networks );
		}
